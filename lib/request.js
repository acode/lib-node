...
function followRedirect(cfg, response, callback) {
  ...
  Object.assign(cfg, {host: parsedUrl.host, path: parsedUrl.pathname, port: parsedUrl.protocol.includes('https') ? 443 : 80});

  return request(cfg, 'GET', '', {}, Buffer.from(JSON.stringify({})), (err, res) => {
    if (err) {
      return callback(err);
    }
...
function request(cfg, method, pathname, headers, body, callback) {
  ...
  let req = (cfg.port === 443 ? https : http).request({
    ...
    res.on('end', () => {
      ...
      if (((res.statusCode / 100) | 0) !== 2 && ((res.statusCode / 100) | 0) !== 3) {
        ...
        return followRedirect(cfg, res, (err, response) => {
          if (err) {
            return callback(err);
          }
