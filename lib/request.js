...
function followRedirect(cfg, response, maxRedirects, callback) {
  if (maxRedirects < 1) {
    throw new Error('Too many redirects');
  }

  let parsedUrl = response.headers.location && url.parse(response.headers.location);

  let updatedCfg = Object.keys(cfg).reduce((n, key) => {
    n[key] = cfg[key];
    return n;
  }, {});

  updatedCfg.host = parsedUrl.host;
  updatedCfg.path = parsedUrl.pathname;
  updatedCfg.port = parsedUrl.protocol.includes('https') ? 443 : 80;

  return request(updatedCfg, 'GET', '', {}, Buffer.from(JSON.stringify({})), [], maxRedirects - 1, (err, res) => {
    if (err) {
      return callback(err);
    }
...
function request(cfg, method, pathname, headers, body, names, maxRedirects, callback) {
  ...
  let req = (cfg.port === 443 ? https : http).request({
    ...
    res.on('end', () => {
      ...
      if (((res.statusCode / 100) | 0) === 2) {
        ...
        return followRedirect(cfg, res, maxRedirects, (err, response) => {
          if (err) {
            return callback(err);
          }
